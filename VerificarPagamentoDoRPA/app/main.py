...
def enviar_email_cobranca(dias_sem_pagar):
    ...
    email.htmlMessage = "O pagamento de Fulano de Tal está com {} dias de atraso".format(
        dias_sem_pagar)
    return email.send()
...
def enviar_email_rpa_pago(valor):
    ...
    email.htmlMessage = "Seu RPA no valor de {} foi pago pela fundação. Favor verificar no seu extrato".format(
        valor)
    return email.send()
...
def enviar_oficio_cobranca(dados):
    ...
    doc1 = DocxTemplate(
        join(templates_dir, 'Oficio_Cobranca_RTVE-Cobranca_RPA.docx'))
    doc1.render(dados.iloc[0])
    outputs_dir = join(expanduser("~"), 'outputs')
    document_path = join(
        outputs_dir, "Oficio_Cobranca_RTVE-Cobranca_RPA_{}.docx".format(n_oficio_cobranca))
    doc1.save(document_path)
    email.toAddresses = ["desenvolvedor1@cett.org.br"]
    email.subject = "RPA - Ofício de cobrança"
    email.htmlMessage = "Segue ofício referente ao pagamento de Fulano de Tal".format(
        dias_sem_pagar)
...
def verifica_pagamento(rpaID):
    ...
    dados['valor_liquido'] = dados['valor_liquido'].str.replace(
        '[R$ .]', '').str.replace('[,]', '.').astype(float)
